...
func TestRetryAfterCooldownSerializesPostDeadlineWaiters(t *testing.T) {
	cooldown := &retryAfterCooldown{}
	response := &http.Response{
		StatusCode: http.StatusTooManyRequests,
		Header:     http.Header{"Retry-After-Ms": []string{"1"}},
	}
	cooldown.observeResponse(response, time.Now().Add(-time.Second), false, 0)
	...
	for range 2 {
		go func() {
			coordinated, _, release, err := cooldown.wait(ctx)
			if err != nil {
				done <- err
				return
			}
			defer release()
			if !coordinated {
				done <- errors.New("waiter did not observe the remembered cooldown")
				return
			}
	...
	select {
	case firstPosition = <-entered:
	case <-ctx.Done():
		t.Fatal("no waiter entered the release gate")
	}
	if firstPosition != 1 {
		t.Fatalf("first released waiter has position %d", firstPosition)
	}
	select {
	...
	for range 2 {
		...
		case <-ctx.Done():
			t.Fatal("waiter did not finish after release")
		}
	}
}

func TestRetryAfterCooldownDoesNotClearNewerGeneration(t *testing.T) {
	cooldown := &retryAfterCooldown{}
	response := &http.Response{
		StatusCode: http.StatusTooManyRequests,
		Header:     http.Header{"Retry-After-Ms": []string{"1"}},
	...
	cooldown.observeResponse(response, time.Now().Add(-time.Second), false, 0)
	coordinated, generation, release, err := cooldown.wait(context.Background())
	if err != nil {
		t.Fatalf("wait returned error for expired cooldown: %v", err)
	}
	if !coordinated {
		t.Fatal("wait bypassed the remembered cooldown")
	}

	cooldown.observeResponse(
		&http.Response{
			StatusCode: http.StatusTooManyRequests,
	...
	_, _, _, err = cooldown.wait(ctx)
	if !errors.Is(err, context.DeadlineExceeded) {
		t.Fatalf("older success cleared a newer cooldown generation: %v", err)
	}
}
