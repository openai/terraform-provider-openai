// Code generated by generate-terraform-provider. DO NOT EDIT.

package grouprole

import (
	"context"
	"fmt"

	openaiapi "github.com/openai/terraform-provider-openai/internal/provider/openaiapi"

	"github.com/hashicorp/terraform-plugin-framework/datasource"
	"github.com/hashicorp/terraform-plugin-framework/datasource/schema"
	"github.com/hashicorp/terraform-plugin-framework/schema/validator"
	"github.com/hashicorp/terraform-plugin-framework/types"
)

var _ datasource.DataSource = &GroupRoleDataSource{}
var _ datasource.DataSourceWithConfigure = &GroupRoleDataSource{}

type GroupRoleDataSource struct {
	client *openaiapi.APIClient
}

type GroupRoleDataSourceModel struct {
	GroupID        types.String `tfsdk:"group_id"`
	RoleID         types.String `tfsdk:"role_id"`
	ID             types.String `tfsdk:"id"`
	Name           types.String `tfsdk:"name"`
	Description    types.String `tfsdk:"description"`
	Permissions    types.List   `tfsdk:"permissions"`
	ResourceType   types.String `tfsdk:"resource_type"`
	PredefinedRole types.Bool   `tfsdk:"predefined_role"`
	ResponseJSON   types.String `tfsdk:"response_json"`
}

func NewGroupRoleDataSource() datasource.DataSource {
	return &GroupRoleDataSource{}
}

func (d *GroupRoleDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = "openai_group_role"
}

func (d *GroupRoleDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Look up an organization-level role assignment for a group.\n\nAuthentication: Set the admin_api_key provider attribute or OPENAI_ADMIN_KEY environment variable for the admin API audience.",
		Attributes: map[string]schema.Attribute{
			"group_id": schema.StringAttribute{
				MarkdownDescription: "group id",
				Required:            true,
				Optional:            false,
				Computed:            false,
				Sensitive:           false,
				Validators: []validator.String{
					openaiapi.StringIsValidPathParameter(),
					openaiapi.StringLengthAtLeast(1),
				},
			},
			"role_id": schema.StringAttribute{
				MarkdownDescription: "role id",
				Required:            true,
				Optional:            false,
				Computed:            false,
				Sensitive:           false,
				Validators: []validator.String{
					openaiapi.StringIsValidPathParameter(),
					openaiapi.StringLengthAtLeast(1),
				},
			},
			"id": schema.StringAttribute{
				MarkdownDescription: "id",
				Required:            false,
				Optional:            false,
				Computed:            true,
				Sensitive:           false,
			},
			"name": schema.StringAttribute{
				MarkdownDescription: "name",
				Required:            false,
				Optional:            false,
				Computed:            true,
				Sensitive:           false,
			},
			"description": schema.StringAttribute{
				MarkdownDescription: "description",
				Required:            false,
				Optional:            false,
				Computed:            true,
				Sensitive:           false,
			},
			"permissions": schema.ListAttribute{
				MarkdownDescription: "permissions",
				ElementType:         types.StringType,
				Required:            false,
				Optional:            false,
				Computed:            true,
				Sensitive:           false,
			},
			"resource_type": schema.StringAttribute{
				MarkdownDescription: "resource type",
				Required:            false,
				Optional:            false,
				Computed:            true,
				Sensitive:           false,
			},
			"predefined_role": schema.BoolAttribute{
				MarkdownDescription: "predefined role",
				Required:            false,
				Optional:            false,
				Computed:            true,
				Sensitive:           false,
			},
			"response_json": schema.StringAttribute{
				MarkdownDescription: "Raw JSON response returned by the OpenAI API.",
				Required:            false,
				Optional:            false,
				Computed:            true,
				Sensitive:           false,
			},
		},
	}
}

func (d *GroupRoleDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	if req.ProviderData == nil {
		return
	}
	providerData, ok := req.ProviderData.(*openaiapi.ProviderData)
	if !ok {
		resp.Diagnostics.AddError("Unexpected Data Source Configure Type", fmt.Sprintf("Expected *openaiapi.ProviderData, got: %T", req.ProviderData))
		return
	}
	client, ok := providerData.Client("admin")
	if !ok {
		resp.Diagnostics.AddError(
			"Missing OpenAI credential for admin API",
			"Set the admin_api_key provider attribute or OPENAI_ADMIN_KEY environment variable for the admin API audience before using openai_group_role. Credentials are not reused across API audiences.",
		)
		return
	}
	d.client = client
}

func (d *GroupRoleDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data GroupRoleDataSourceModel
	resp.Diagnostics.Append(req.Config.Get(ctx, &data)...)
	if resp.Diagnostics.HasError() {
		return
	}
	if d.client == nil {
		resp.Diagnostics.AddError("Missing OpenAI API client", "The provider was not configured before this operation ran.")
		return
	}
	pathParams := map[string]string{
		"group_id": data.GroupID.ValueString(),
		"role_id":  data.RoleID.ValueString(),
	}
	queryParams := map[string]string{}
	responseData, err := d.client.Request(ctx, "GET", "/organization/groups/{group_id}/roles/{role_id}", pathParams, queryParams, nil)
	if err != nil {
		resp.Diagnostics.AddError("OpenAI API request failed", err.Error())
		return
	}
	data.ID = types.StringValue(fmt.Sprintf("openai_group_role/%s/%s", data.GroupID.ValueString(), data.RoleID.ValueString()))
	if err := openaiapi.ApplyStringResponseField(responseData, []string{"id"}, &data.ID, true); err != nil {
		resp.Diagnostics.AddError("Invalid OpenAI API response", err.Error())
		return
	}
	if err := openaiapi.ApplyStringResponseField(responseData, []string{"name"}, &data.Name, false); err != nil {
		resp.Diagnostics.AddError("Invalid OpenAI API response", err.Error())
		return
	}
	if err := openaiapi.ApplyStringResponseField(responseData, []string{"description"}, &data.Description, false); err != nil {
		resp.Diagnostics.AddError("Invalid OpenAI API response", err.Error())
		return
	}
	if err := openaiapi.ApplyStringListResponseField(responseData, []string{"permissions"}, &data.Permissions, false); err != nil {
		resp.Diagnostics.AddError("Invalid OpenAI API response", err.Error())
		return
	}
	if err := openaiapi.ApplyStringResponseField(responseData, []string{"resource_type"}, &data.ResourceType, false); err != nil {
		resp.Diagnostics.AddError("Invalid OpenAI API response", err.Error())
		return
	}
	if err := openaiapi.ApplyBoolResponseField(responseData, []string{"predefined_role"}, &data.PredefinedRole, false); err != nil {
		resp.Diagnostics.AddError("Invalid OpenAI API response", err.Error())
		return
	}
	data.ResponseJSON = openaiapi.ResponseJSONValue(responseData)
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}
