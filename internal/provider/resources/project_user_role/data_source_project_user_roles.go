...
var _ datasource.DataSourceWithConfigure = &ProjectUserRolesDataSource{}
...
type ProjectUserRolesDataSourceItemModel struct {
	ID             types.String `tfsdk:"id"`
...
func (ProjectUserRolesDataSourceItemModel) AttributeTypes() map[string]attr.Type {
...
func (d *ProjectUserRolesDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = "openai_project_user_roles"
}
...
func (d *ProjectUserRolesDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	responseData, err := d.client.PaginatedRequest(ctx, "GET", "/projects/{project_id}/users/{user_id}/roles", pathParams, queryParams)
	if err != nil {
		resp.Diagnostics.AddError("OpenAI API request failed", err.Error())
		return
	}
	data.ID = types.StringValue(fmt.Sprintf("openai_project_user_roles/%s/%s", data.ProjectID.ValueString(), data.UserID.ValueString()))
	...
	items := []ProjectUserRolesDataSourceItemModel{}
	for _, itemData := range resultItems {
		item := ProjectUserRolesDataSourceItemModel{}
	...
	itemsValue, diags := types.ListValueFrom(ctx, types.ObjectType{AttrTypes: ProjectUserRolesDataSourceItemModel{}.AttributeTypes()}, items)
